...
export function AiItineraryPage() {
  ...
  const updateAiItem = (dayIndex: number, itemIndex: number, patch: Partial<AiItinerary["days"][number]["items"][number]>) => {
    setItinerary((current) => {
      ...
      const daysCopy = current.days.map((day, dIndex) => {
        if (dIndex !== dayIndex) return day;
        return {
          ...day,
          items: day.items.map((item, iIndex) => iIndex === itemIndex ? { ...item, ...patch } : item),
        };
      });
      return { ...current, days: daysCopy };
    });
  };
  const removeAiItem = (dayIndex: number, itemIndex: number) => {
    setItinerary((current) => current ? {
      ...
      days: current.days.map((day, dIndex) => dIndex === dayIndex
        ? { ...day, items: day.items.filter((_, iIndex) => iIndex !== itemIndex) }
        : day),
    } : current);
  };
  ...
  return (
    <section className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="text-4xl font-semibold">AI tạo lịch trình</h1>
      ...
      <Card className="mt-6">
        <CardContent className="grid gap-4 pt-5">
          ...
          <Button className="w-fit" onClick={() => void handleGenerate()} disabled={loading || interests.length === 0}>
      ...
      {itinerary && (
        <div className="mt-8 grid gap-5">
          ...
          {itinerary.days.map((day, dayIndex) => (
            <Card key={day.day_number}>
              <CardContent className="pt-5">
                <h2 className="text-xl font-semibold">Ngày {day.day_number}</h2>
                <p className="mt-1 text-sm text-muted-foreground">{day.summary}</p>
                <div className="mt-4 grid gap-3">
                  {day.items.length === 0 ? (
                    <p className="text-sm text-muted-foreground">Ngày này chưa có hoạt động.</p>
                    ...
                    <div className="grid gap-2 rounded-md border p-3 md:grid-cols-[120px_1fr_auto]" key={`${day.day_number}-${itemIndex}`}>
                      ...
                      <div className="grid gap-2">
                        ...
                        <Input value={item.transport_note ?? ""} placeholder="Ghi chú di chuyển" onChange={(event) => updateAiItem(dayIndex, itemIndex, { transport_note: event.target.value })} />
                      </div>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Button variant="outline" onClick={() => navigate("/itineraries/new")}>Chỉnh sửa thủ công</Button>
          </div>
        </div>
      )}
    </section>
