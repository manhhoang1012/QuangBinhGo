...
export function MyPostsPage() {
  ...
  return (
    <section className="mx-auto max-w-5xl px-4 py-10">
      ...
      <div className="mt-8 grid gap-4">{posts.map((post) => <Card key={post.id}><CardContent className="pt-5"><p className="text-sm text-muted-foreground">{post.place.name}</p><h2 className="mt-2 text-xl font-semibold">{post.title}</h2><p className="mt-2 text-sm text-muted-foreground">{post.content}</p></CardContent></Card>)}</div>
    </section>
  );
}
